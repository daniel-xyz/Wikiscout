<template>
  <header>
    ...
    <nav id="main-nav">
      <div class="hide-md-and-up menu-toggle-icon" @click="menuExpanded = !menuExpanded" @mouseleave="menuExpanded = false">
        <transition name="fade">
          <div class="fullscreen-bg-layer" v-show="menuExpanded">
            <ul>
              <router-link :to="{ path: '/data' }"><li>Datenschutz</li></router-link>
              <router-link :to="{ path: '/imprint' }"><li>Impressum</li></router-link>
            </ul>
          </div>
        </transition>
      </div>
